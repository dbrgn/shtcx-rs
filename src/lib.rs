...
impl MeasurementDuration for sensor_class::Sht1Gen {
    ...
    fn max_measurement_duration(mode: PowerMode) -> u32 {
        match mode {
            PowerMode::NormalMode => 14_400_000,
            PowerMode::LowPower => 940_000,
        }
    }
}
    ...
    I2C::Error: Into<Error<I2C::Error>>,
{
    ...
    pub fn wait_for_measurement(&mut self, mode: PowerMode, delay: &mut impl DelayNs) {
        delay.delay_ns(S::max_measurement_duration(mode));
    }
...
mod tests {
    ...
    mod max_measurement_duration {
        ...
        fn shortcut_function() {
            ...
            assert_eq!(
                max_measurement_duration(&c1, PowerMode::NormalMode),
                14_400_000
            );
            assert_eq!(max_measurement_duration(&c1, PowerMode::LowPower), 940_000);
